      ...
      <div className="space-y-4">
        ...
        {activeCategory === 'mbti' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Object.values(MBTI_PROFILES).map((p) => (
        ...
        {activeCategory === 'enneagram' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {Object.values(ENNEAGRAM_CORE_PROFILES).map((p) => (
              <M3Card key={p.type} variant="outlined" className="p-5 bg-surface-container">
        ...
        {activeCategory === 'instinct' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Object.values(INSTINCT_PROFILES).map((p) => (
        ...
        {activeCategory === 'jungian' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Object.values(JUNGIAN_FUNCTIONS_INFO).map((f) => (
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              {Object.keys(QUADRA_DETAILS).map((qKey) => {
                const q = QUADRA_DETAILS[qKey];
                return (
                  <div key={qKey} className="p-4 rounded-m3-lg bg-surface-container border border-primary/20">
                    <h4 className="font-bold text-sm text-primary mb-1">{q.title[language]}</h4>
                    <p className="text-[11px] text-on-surface-variant">{q.values[language]}</p>
                  </div>
                );
              })}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
              {Object.values(SOCIOTYPES).map((s) => (
        ...
        {activeCategory === 'attitudinal_psyche' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
              {Object.entries(AP_TYPE_ARCHETYPES).map(([type, arch]) => (
        ...
        {activeCategory === 'big5' && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {Object.values(BIG5_DIMENSIONS).map((dim) => (
        ...
        {activeCategory === 'alignment' && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {Object.values(ALIGNMENT_DETAILS).map((al) => (
